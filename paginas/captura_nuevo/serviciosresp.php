		...
		<?php foreach ($resultado as $res) { ?>
			<?php if($res->tipo_sv==1){ ?>
				<div class="pull-left" style="border-style: groove;border-width: 3px;max-height: 100%;height: 100%;width:60%;max-width: 60% ">
					<label class="copy" for="precio_<?php echo $servicio->id_servicio ?>_cb"  style="width:100%;max-width:100%;margin:0;height: 70%;max-height: 70%;color:black">
					...
					<input type="number" style="width: 100%;display: none;" name="precio_<?php echo $servicio->id_servicio ?>" id="precio_<?php echo $servicio->id_servicio ?>" value="<?php if(isset($res->cantidad_sv)){ echo $res->cantidad_sv;}else {echo 0;} ?>"> 
					<?php if(isset($res->cantidad_sv) && $res->cantidad_sv>0){ 
						$checked="checked";
					}else {
						$checked="";
					}
					?>
					<input type="checkbox" id="precio_<?php echo $servicio->id_servicio ?>_cb" value="1" <?php echo $checked; ?> >
				</div>
			<?php } else{ ?>
				<div class="pull-right" style="border-style: groove;border-width: 3px;max-height: 100%;height: 100%;width:40%;max-width: 40%;vertical-align: middle;">
					<?php if($servicio->cortesia_servicio==1){ ?>
						<label class="copy" for="cortesia_<?php echo $servicio->id_servicio ?>_cb"  style="width:100%;max-width:100%;margin:0;height: 60%;max-height: 100%;color:black">
						<small>Cortesia</small>		
						</label>
						<input type="number" style="width: 100%;display: none;" name="cortesia_<?php echo $servicio->id_servicio ?>" id="cortesia_<?php echo $servicio->id_servicio ?>" value="<?php if(isset($res->cantidad_sv)){ echo $res->cantidad_sv;}else {echo 0;} ?>">
						<?php if(isset($res->cantidad_sv) && $res->cantidad_sv>0){ 
							$checked2="checked";
						}else {
							$checked2="";
						} ?>		
						<input type="checkbox" id="cortesia_<?php echo $servicio->id_servicio ?>_cb" <?php echo $checked2 ?> onclick="cargar_check(this)">				
					<?php } else{?>
